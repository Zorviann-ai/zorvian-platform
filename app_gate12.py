...
def autonomy_run(body: AutonomyRunIn, request: Request, u=Depends(current_user)):
    ...
    return {"run_id": run_id, "mode": policy["mode"], "before": before.as_dict(), "after": after.as_dict(), "actions": actions, "assessment": assessment, "provider": provider, "human_approval_required": approval_required}
